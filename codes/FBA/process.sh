...
run 'segmenting FOD images to estimate fixels and their fiber density' \
 fod2fixel -mask IN:$output_folder/$warped_mask_average IN:$fod_in_template_space_NOT_REORIENTED OUT:$fixel_in_template_space_NOT_REORIENTED -afd OUT:$fd

run 'reorienting direction of fixels' \
 fixelreorient IN:$fixel_in_template_space_NOT_REORIENTED IN:$native2average_warp OUT:$fixel_in_template_space

cd $output_folder
id_ses=$(echo $ID | sed 's/\//_/')
run 'running fixel correspondence and computing fiber density'\
 fixelcorrespondence IN:$ID/$fixel_in_template_space/$fd IN:$fixel_mask OUT:$all_subj_fd OUT:${id_ses}.mif

run 'computing fibre cross-section' \
