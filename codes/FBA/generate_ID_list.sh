...
run 'generate all available ID list' \
  python generate_ID_list.py --folder IN:$rf_data --pattern sub*/ses* \| python generate_ID_list.py --folder IN:$dwi_data --pattern sub*/ses* \| python generate_ID_list.py --file IN:$participants_info --columns 0 2 --prefix sub- ses- --apcolumns 5 3 4 --out OUT:all_available_IDs.txt

#applying criteria

#dHCP_participants_info=$(python generate_ID_list.py -file $participants_info -columns 0 2 -prefix sub- ses- -apcolumns 5 3 4) # 5 - Gender 3- GA at birth 4 - PMA
#dwi_participants_info=$(python generate_ID_list.py -folder $dwi_data -pattern sub*/ses*)
#rf_participants_info=$(python generate_ID_list.py -folder $rf_data -pattern sub*/ses*)
#
#comm -12 <( sort $dwi_participants_info) <( sort $rf_participants_info)
#

#usage example generate_ID_list data/ 'sub*/*' 
#
